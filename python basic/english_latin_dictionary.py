...
(that is, all English words, for which our Latin word is among its translations), and take them
and only them as the translations of this Latin word.
...
Followed by N dictionary entries. Each entry is contained on a separate line, which contains first the English word,
then a hyphen surrounded by spaces and then comma-separated list with the translations of this English word in Latin.
...
for _ in range(int(input())):
    eng_word, *l_words = input().replace(' -', ',').split(', ')
    for l_word in l_words:
        dictionary[l_word] = dictionary.get(l_word, []) + [eng_word]

print(len(dictionary))

for key in sorted(dictionary):
    print(key, ', '.join(dictionary[key]), sep=' - ')
